...
def boas_vindas(message):
    user_id = message.chat.id
    with open("video_boas_vindas.mp4", "rb") as video:
...
def comprar_handler(call):
    ...
    bot.send_message(user_id, "💳 Gerando sua cobrança PIX... Aguarde 🔄")

    url = "https://www.asaas.com/api/v3/payments"
    headers = {"accept": "application/json", "content-type": "application/json", "access_token": API_TOKEN}
    due_date = (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")
    try:
        customer_id = criar_cliente_asaas(user_id, nome)
        payload = {
            "billingType": "PIX",
        ...
        bot.send_message(user_id, "❌ Ocorreu um erro. Tente novamente mais tarde.")
        ...
        try:
            for user_id, dados in list(cobrancas_pendentes.items()):
                ...
                if res.get("status") == "RECEIVED":
                    bot.send_message(user_id, "✅ *Pagamento confirmado!*\n\nAcesse seu conteúdo VIP por 7 dias:\n👉 https://t.me/+iN6NGTm_LMtlNTYx", parse_mode="Markdown")
                    try:
                        bot.approve_chat_join_request(CANAL_CHAT_ID, user_id)
                    except:
                        pass
                    bot.send_message(user_id, "⌛ Em 7 dias seu acesso expira. Para renovar, use: https://t.me/ConteudoVipBot?start=renovar")
                    del cobrancas_pendentes[user_id]
        except Exception as e:
            print(f"Erro na verificação de pagamento: {e}")

        time.sleep(30)  # Pode ajustar se quiser
...
if __name__ == "__main__":
    ...
    print("Webhook configurado em:", WEBHOOK_URL)
