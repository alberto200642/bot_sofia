...
def comprar_handler(call):
    ...
    bot.send_message(
        user_id,
        f"💳 *Pagamento via PIX*\n\nEfetue o pagamento de *R$ {PRECO:.2f}* usando o código abaixo.\n\n🔁 Após o pagamento, clique em *'Já paguei'*. Liberação em até 1 minuto.",
        parse_mode="Markdown")
    bot.send_message(user_id,
                     f"📲 *Copia e Cola PIX:*\n\n`{pix_copiaecola}`",
                     parse_mode="Markdown")
    ...
    markup = telebot.types.InlineKeyboardMarkup()
    btn1 = telebot.types.InlineKeyboardButton("✅ Já paguei",
                                              callback_data="paguei")
    markup.add(btn1)
    bot.send_message(user_id,
                     "Após o pagamento, clique abaixo 👇",
                     reply_markup=markup)
    ...
    while True:
        for user_id, dados in list(cobrancas_pendentes.items()):
            ...
            if res.get("status") == "RECEIVED":
                ...
                bot.send_message(
                    user_id,
                    "⌛ Em 7 dias seu acesso expira. Para renovar, use este link: https://t.me/ConteudoVipBot?start=renovar"
                )
